...
void usage()
{
  std::cout << "usage: dslviz map.stl start_x start_y start_z goal_x goal_y goal_z cells_per_meter samples_per_face [--orient] [--height_penalty] [--georef lat1 lon1 x1 y1 lat2 lon2 x2 y2] [--save_ways out.csv lat lon x y]" << std::endl; 
}

nav_msgs::Path dsl_path_to_ros_msg(GridPath3D dsl_path, float cellsPerMeter, float xmin, float ymin, float zmin)
{
  nav_msgs::Path msg;  
...
void xypath_to_waycsv(nav_msgs::Path path, double lat, double lon, Eigen::Vector2d v, double zstart_ref, std::string filename)
{
  ...
  for(unsigned int i = 0; i < path.poses.size(); i++)
  {
    outf << i+1 << ";" 
         ...
         << std::fixed << std::setprecision(3) << (path.poses[i].pose.position.z-zstart_ref)*10 << ";"
         << "0.000;" << "0.50;" << "3.000;" << "23;" << std::endl;
  }
